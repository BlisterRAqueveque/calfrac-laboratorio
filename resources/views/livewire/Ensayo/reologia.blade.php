   @if ($lechada->rel_reologia)
       ...
       <div class="accordion" id="accordionExample">
           @php
               ...
               $form_reologia = true;
           @endphp
           @foreach ($rel_reologia as $reologia)
               <div class="accordion-item">
                   <h2 class="accordion-header">
                       <button
                           class="accordion-button collapsed p-2 {{ $reologia->selected == 1 ? 'bg-green-300 text-green-600 font-semibold border border-green-600 bg-opacity-40' : '' }}"
                           type="button" data-bs-toggle="collapse" data-bs-target="#reologia_{{ $reologia->id }}"
                           aria-expanded="false" aria-controls="reologia_{{ $reologia->id }}">
                       ...
                       <div class="accordion-body overflow-auto">
                           <div class="flex justify-between items-center mb-3">
                               ...
                               <div class="flex items-center gap-3">
                                   ...
                                   <form id="form_assignment_{{ $reologia->id }}" wire:submit.prevent="store">
                                       @csrf
                                       <input type="hidden" wire:model="id_assignment" value="{{ $reologia->id }}">
   ...
   {{-- Cuando se escucha por el botón de "Crear Registro" muestra un sweetalert para realizar la carga de la reología --}}
   <script>
       let btn_click_reologia = document.getElementById('btn_click_reologia');
       if (btn_click_reologia) {
           btn_click_reologia.addEventListener('click', e => {
               e.preventDefault();
               confirmAlert().then((confirmed) => {
                   if (confirmed) {
                       document.getElementById('submit_btn_reologia').click()
                       successAlert('¡Reología Cargada!', 'El registro se creó correctamente.')
                   }
               })
           })
       }
