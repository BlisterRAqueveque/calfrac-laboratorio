<div class="mt-4 tab-pane fade" id="tab_agua_libre" role="tabpanel"> <!-- Agua Libre -->
    @if ($solicitud_lechada[0]->rel_agua_libre)
        <div class="mb-2 text-center">
            <h5 class="mb-1">Registros de Agua Libre</h5>
        </div>
        <div class="accordion" id="accordionAguaLibre">
            @php
                $i = 1;
                $form_agua_libre = true;
            @endphp

            @foreach ($solicitud_lechada[0]->rel_agua_libre as $agua_libre)
                @php
                    if ($agua_libre->selected == 1) {
                        $form_agua_libre = false;
                    }
                @endphp
                <div class="accordion-item">
                    <h2 class="accordion-header {{ $agua_libre->selected ? 'bg-green-50' : '' }}">
                        <button class="accordion-button collapsed p-2" type="button" data-bs-toggle="collapse"
                            data-bs-target="#agua_libre_{{ $agua_libre->id }}" aria-expanded="false"
                            aria-controls="agua_libre_{{ $agua_libre->id }}">
                            Agua Libre - Intento Nº{{ $i }}
                        </button>
                    </h2>
                    <div id="agua_libre_{{ $agua_libre->id }}" class="accordion-collapse collapse"
                        data-bs-parent="#accordionAguaLibre">
                        <div class="accordion-body overflow-auto">
                            <div class="flex justify-between items-center mb-3">
                                <p class="flex items-center text-gray-700 mb-0">
                                    ...
                                    Registro creado por {{ $agua_libre->user->nombre }}
                                    {{ $agua_libre->user->apellido }} el día
                                    {{ $agua_libre->created_at->format('d') }}
                                    de {{ $agua_libre->created_at->format('M') }},
                                    {{ $agua_libre->created_at->format('Y') }} a las
                                    {{ $agua_libre->created_at->format('H:i') }} hs
                                </p>
                                ...
                                <div class="flex items-center gap-3">
                                    ...
                                    <form id="form_assignment_{{ $agua_libre->id }}">
                                        @csrf
                                        <input type="hidden" name="type_of_assignment" value="agua_libre">
                                        <input type="hidden" name="id_assignment" value="{{ $agua_libre->id }}">
                                        <x-button data-form="form_assignment_{{ $agua_libre->id }}"
                            ...
                            <div class="grid grid-cols-3 gap-3 my-3">
                                <div>
                                    <label class="text-sm text-gray-700 font-semibold tracking-wide mb-2">Agua Libre
                                        °C</label>
                                    <input type="number" value="{{ $agua_libre->agua_libre }}"
                                ...
                                <div>
                                    ...
                                    <input type="number" class="form-control text-sm p-2"
                                        value="{{ $agua_libre->volumen ? $agua_libre->volumen : '0' }}" readonly>
                                </div>
                                <div>
                                    <label class="text-sm text-gray-700 font-semibold tracking-wide mb-2">API Agua Libre
                                        %</label>
                                    <input type="number" class="form-control text-sm p-2"
                                        value="{{ $agua_libre->api_agua_libre ? $agua_libre->api_agua_libre : '0' }}"
                                        readonly>
                                </div>
                            </div>
                        </div>
            ...
            <div class="grid grid-cols-3 gap-3 mb-3">
                ...
                <div>
                    <label for="agua_libre_volumen"
                        class="text-sm text-gray-700 font-semibold tracking-wide mb-2">Volumen mL</label>
